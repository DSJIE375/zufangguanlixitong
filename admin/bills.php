...
if (!empty($_GET['month'])) {
    ...
    $where .= " AND b.bill_month = '$month'";
}
if (!empty($_GET['bill_status'])) {
    $bill_status = sanitize($_GET['bill_status']);
    $where .= " AND b.status = '$bill_status'";
}

$bills = $conn->query("SELECT b.*, c.monthly_rent, r.room_number, t.name as tenant_name
                       FROM bills b
                       JOIN contracts c ON b.contract_id = c.id
                       JOIN rooms r ON c.room_id = r.id
                       JOIN tenants t ON c.tenant_id = t.id
...
if ($action == 'edit' && $id) {
    ...
    $editBill = $result->fetch_assoc();
}

$preselect_contract = $_GET['contract_id'] ?? '';
?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    ...
    <title>水电账单 - <?php echo $siteName; ?></title>
                ...
                <div class="card shadow-sm mb-4">
                    <div class="card-body">
                        <form method="GET" class="row g-3">
                            <div class="col-md-3">
                                <input type="text" name="search" class="form-control" placeholder="搜索房间号或租客..." value="<?php echo $_GET['search'] ?? ''; ?>">
                            </div>
                            <div class="col-md-2">
                                <input type="month" name="month" class="form-control" value="<?php echo $_GET['month'] ?? ''; ?>">
                            </div>
                            <div class="col-md-2">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select name="contract_id" class="form-select" required id="contractSelect" <?php echo $action == 'edit' ? 'disabled' : ''; ?>>
                                                ...
                                                <option value="<?php echo $contract['id']; ?>"
                                                    ...
                                                    <?php echo $contract['room_number']; ?> - <?php echo $contract['tenant_name']; ?> (¥<?php echo number_format($contract['monthly_rent'], 2); ?>/月)
                                                </option>
                                                <?php endwhile; ?>
                                            </select>
                                        </div>
